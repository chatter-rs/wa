...
pub enum TokenIndexError {
    #[error("index of single byte token out of bound: got {index}, limit: {limit}")]
    SingleByteIndexOutOfBounds { index: u8, limit: u8 },
    #[error(
        "dictionary index of double byte token out of bound: got {dict_index}, limit: {limit}"
    )]
    DoubleByteDictionaryOutOfBounds { dict_index: u8, limit: u8 },
    #[error(
        "index of double byte token out of bound in dict {dict_index}: got {index}, limit: {limit}"
    )]
    DoubleByteIndexOutOfBounds {
        dict_index: u8,
        index: u8,
        limit: u8,
    },
...
/// [`get_single_token`] gets the string value of the single byte token at the given index.
/// In case of error, it returns [`TokenIndexError::SingleByteIndexOutOfBounds`].
pub fn get_single_token(index: u8) -> Result<&'static str, TokenIndexError> {
    let index = index as usize;

    if index >= SINGLE_BYTE_TOKENS.len() {
        Err(TokenIndexError::SingleByteIndexOutOfBounds {
            index: index as u8,
            limit: SINGLE_BYTE_TOKENS.len() as u8,
        })
    } else {
        Ok(SINGLE_BYTE_TOKENS[index])
    }
}

/// [`get_double_token`] gets the string value of the double byte token at the given indices.
/// In case of error, it returns either [`TokenIndexError::DoubleByteDictionaryOutOfBounds`]
/// or [`TokenIndexError::DoubleByteIndexOutOfBounds`]
pub fn get_double_token(index1: u8, index2: u8) -> Result<&'static str, TokenIndexError> {
    let index1 = index1 as usize;
    let index2 = index2 as usize;

    if index1 >= DOUBLE_BYTE_TOKENS.len() {
        Err(TokenIndexError::DoubleByteDictionaryOutOfBounds {
            dict_index: index1 as u8,
            limit: DOUBLE_BYTE_TOKENS.len() as u8,
        })
    } else if index2 >= DOUBLE_BYTE_TOKENS[index1].len() {
        Err(TokenIndexError::DoubleByteIndexOutOfBounds {
            dict_index: index1 as u8,
            index: index2 as u8,
            limit: DOUBLE_BYTE_TOKENS.len() as u8,
        })
    } else {
        Ok(DOUBLE_BYTE_TOKENS[index1][index2])
    }
}

pub struct TokenIndexMap {
    single_byte: HashMap<&'static str, u8>,
    double_byte: HashMap<&'static str, (u8, u8)>,
}

impl TokenIndexMap {
    /// [`Self::index_of_single_token`] gets the index of the single-byte token with given string value.
    /// If there is no token with the given value, then it returns [`Option::None`]
    pub fn index_of_single_token(&self, token: &str) -> Option<u8> {
        self.single_byte.get(token).copied()
    }

    /// [`Self::index_of_double_token`] gets the indices (dictionary number, index) of the double-byte
    /// token with given string value.
    /// If there is no token with the given value, then it returns [`Option::None`]
    pub fn index_of_double_token(&self, token: &str) -> Option<(u8, u8)> {
        self.double_byte.get(token).copied()
    }
}

/// [`token_indices_map`] returns a [`TokenIndexMap`] which gets created first time when you call the
/// function and then caches it.
pub fn token_indices_map() -> &'static TokenIndexMap {
    static INDEX_MAP: OnceLock<TokenIndexMap> = OnceLock::new();
    INDEX_MAP.get_or_init(|| {
        let mut single_byte_index = HashMap::with_capacity(SINGLE_BYTE_TOKENS.len());

        for (index, token) in SINGLE_BYTE_TOKENS.into_iter().enumerate() {
            if !token.is_empty() {
                single_byte_index.insert(token, index as u8);
            }
        }

        let double_byte_tokens_count = DOUBLE_BYTE_TOKENS.len() * DOUBLE_BYTE_TOKENS[0].len();
        let mut double_byte_index = HashMap::with_capacity(double_byte_tokens_count);

        for (dict_index, tokens) in DOUBLE_BYTE_TOKENS.into_iter().enumerate() {
            for (index, token) in tokens.into_iter().enumerate() {
                double_byte_index.insert(token, (dict_index as u8, index as u8));
            }
        }

        TokenIndexMap {
            single_byte: single_byte_index,
            double_byte: double_byte_index,
        }
    })
}
...
mod tests {
    ...
    #[test]
    fn index_of_single_token_found() {
        let index_map = token_indices_map();
        let index = index_map.index_of_single_token("xmlstreamstart");
        assert_eq!(index, Some(1))
    }

    #[test]
    fn index_of_single_token_not_found() {
        let index_map = token_indices_map();
        let index = index_map.index_of_single_token("akshettrj");
        assert_eq!(index, None)
    }

    #[test]
    fn index_of_double_token_found() {
        let index_map = token_indices_map();
        let index = index_map.index_of_double_token("reject");
        assert_eq!(index, Some((1, 0)))
    }

    #[test]
    fn index_of_double_token_not_found() {
        let index_map = token_indices_map();
        let index = index_map.index_of_double_token("akshettrj");
        assert_eq!(index, None)
    }
}
